...
import { modelLeaderboardToCsv } from "../../../lib/listExport";
...
export async function GET(request: Request) {
    ...
    const rows = buildModelLeaderboard(filteredIndex, { linkFilters: filters });

    if (format === "csv") {
        const csv = modelLeaderboardToCsv(rows);
        return new Response(csv, {
            headers: {
                "Content-Type": "text/csv; charset=utf-8",
                "Content-Disposition":
                    'attachment; filename="model-leaderboard.csv"',
